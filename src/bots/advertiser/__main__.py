...
from src.bots.advertiser.handlers import get_root_router
from src.bots.advertiser.middlewares import (
...
async def main() -> None:
    ...
    for observer in (dp.message, dp.callback_query, dp.chat_member):
        ...
        observer.outer_middleware(adv_mw)

    dp.include_router(get_root_router())

    # Initialize checker bot pool (needed for chat validation in campaign-create flow)
    ...
    log.info("advertiser_bot_starting")
    ...
    try:
        ...
        log.info("advertiser_bot_stopped")
