...
class Settings(BaseSettings):
    ...
    # --- TON (прямое пополнение USDT-jetton на собственный кошелёк) ---
    ton_wallet_address: str = Field(
        default="UQDi7UzDfC4a1MKxo1CceHUDPcn9yRbXH4iHFhHOY0h1synZ",
        description="TON-кошелёк, на который принимаются прямые пополнения USDT",
    )
    ton_usdt_jetton_master: str = Field(
        default="EQCxE6mUtQJKFnGfaROTKOt1lZbDiiX1kCixRv7Nw2Id_sDs",
        description="Адрес jetton-мастера USDT (USD₮) в mainnet TON",
    )
    ton_api_base_url: str = Field(default="https://tonapi.io")
    ton_api_key: SecretStr | None = Field(
        default=None, description="Ключ tonapi.io (опционален; поднимает лимиты)"
    )
    ton_topup_expires_minutes: int = Field(default=60)
    ton_amount_tolerance_percent: Decimal = Field(
        default=Decimal("2"), description="Допуск недоплаты USDT из-за дрейфа курса, %"
    )

    # --- GeoIP (определение страны/города и VPN по IP на веб-регистрации) ---
    geoip_enabled: bool = Field(
        default=True, description="Включить определение гео по IP при онбординге"
    )
    geoip_api_base_url: str = Field(
        default="http://ip-api.com/json",
        description="База geoip-провайдера. ip-api.com (free) отдаёт proxy/hosting-флаги для VPN-детекта",
    )
    geoip_api_key: SecretStr | None = Field(
        default=None, description="Ключ ip-api Pro (опционален; free-тариф работает без ключа)"
    )
    geoip_timeout_seconds: float = Field(default=3.0)
